...
class DeDeuplication(logger:Logger,hashMap: Map[String, String],curTime:String,historyTable:String,dedupTab:String,schema:String) extends sparkEngine {
  ...
  /**
    ...
    *
    * @dataFrame deDupWithInFile funcation rquired dataframe as an input
    *
    */
    private val currentTS=fileOper.getCurrentTimeStamp


    def deDupOnFileContain(fileName:String):DataFrame= {
    ...
    //sparkSession.sqlContext.sql("select * from devl_us9_mmi_batch.problem_list_err")
      ...
      sourDF = sparkSession.sqlContext.sql(s"SELECT * from $schema.$historyTable where checkSum='$checkSum'").cache()
      val countVal:Long = sparkSession.sqlContext.sql(s"SELECT * from $schema.$historyTable where checkSum='$checkSum'").cache().count()
      ...
      if (countVal == 0) {
        ...
        sparkSession.sqlContext.sql(s"insert into $schema.$dedupTab select $checkSum,$fileName,$currentTS ")
  ...
  /**
    ...
    *
    */
    def deDupWithInFileNAcrossDB(fileName:String,tableName:String,propertyObj:ReadingConfigFile):DataFrame= {
      ...
      try {
        ...
        if (duplicateDF.count() > 0) {
          ...
          tagDF=sparkSession.sqlContext.sql(s"select * from $schema.$historyTable").cache()
          tagDF=tagDF.withColumn("md5_32bitCol", md5(concat_ws("||", DF.columns.map(col): _*))).cache()
          freeDF=sparkSession.sqlContext.sql(s"select * from sourTable s where not exists(select 1 from $schema.$historyTable h where s.md5_32bitCol=h.md5_32bitCol)")
          .drop("md5_32bitCol")
          .cache()
          ...
          sparkSession.sqlContext.sql(s"insert into $schema.$dedupTab select md5_32bitCol,$fileName,$currentTS from freeTable")
          //Process to get duplicate records//
          freeDFDeDup=sparkSession.sqlContext.sql(s"select * from sourTable s where exists(select 1 from  $schema.$historyTable h where h.md5_32bitCol=g.md5_32bitCol) ").cache()
          .drop("md5_32bitCol")
          .cache()
          freeDFDeDup.createOrReplaceTempView("freeTable")
          sparkSession.sqlContext.sql(s"insert into $schema.$dedupTab select md5_32bitCol,$fileName,$currentTS from freeTable").cache()
